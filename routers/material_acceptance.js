...
router.post('/', authToken, async (req, res) => {
    try {
        const record = await MaterialAcceptance.create(req.body);
...
router.post('/sync', authToken, async (req, res) => {
    try {
        ...
        for (const item of records) {
            const existing = await MaterialAcceptance.findOne({ where: { clientId: item.clientId } });
            if (existing) await existing.update(item);
            else results.push(await MaterialAcceptance.create(item));
        }
...
router.get('/', authToken, async (req, res) => {
    try {
        const { page = 1, pageSize = 20, materialName, supplierName, startDate, endDate } = req.query;
        const where = {};
        ...
        const { count, rows } = await MaterialAcceptance.findAndCountAll({
            where,
...
router.get('/:id', authToken, async (req, res) => {
    try {
        const record = await MaterialAcceptance.findByPk(req.params.id);

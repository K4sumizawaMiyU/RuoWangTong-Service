...
router.post('/', authToken, async (req, res) => {
    try {
        const record = await EquipmentLog.create(req.body);
...
router.post('/sync', authToken, async (req, res) => {
    try {
        ...
        for (const item of records) {
            const existing = await EquipmentLog.findOne({ where: { clientId: item.clientId } });
            if (existing) await existing.update(item);
            else results.push(await EquipmentLog.create(item));
        }
...
router.get('/', authToken, async (req, res) => {
    try {
        const { page = 1, pageSize = 20, equipmentId, startDate, endDate } = req.query;
        const where = {};
        if (equipmentId) where.equipmentId = { [Op.like]: `%${equipmentId}%` };
        if (startDate && endDate) {
            where.recordTime = { [Op.between]: [new Date(startDate), new Date(endDate)] };
        }
...
router.get('/:id', authToken, async (req, res) => {
    try {
        const record = await EquipmentLog.findByPk(req.params.id);

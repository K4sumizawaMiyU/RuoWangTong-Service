...
router.post('/sync', authToken, async (req, res) => {
    try {
        ...
        for (const item of records) {
            const existing = await InspectionRecord.findOne({ where: { clientId: item.clientId } });
            if (existing) await existing.update(item);
            else results.push(await InspectionRecord.create(item));
        }
...
router.get('/', authToken, async (req, res) => {
    try {
        const { page = 1, pageSize = 20, inspectorName, inspectionArea, startDate, endDate } = req.query;
        const where = {};
        if (inspectorName) where.inspectorName = { [Op.like]: `%${inspectorName}%` };
        if (inspectionArea) where.inspectionArea = { [Op.like]: `%${inspectionArea}%` };
        if (startDate && endDate) {
            where.inspectionTime = { [Op.between]: [new Date(startDate), new Date(endDate)] };
        }
...
router.get('/:id', authToken, async (req, res) => {
    try {
        const record = await InspectionRecord.findByPk(req.params.id);

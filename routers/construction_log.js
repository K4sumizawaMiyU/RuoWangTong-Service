...
router.post('/', authToken, async (req, res) => {
    try {
        const data = req.body;
        const record = await ConstructionLog.create(data);
...
router.post('/sync', authToken, async (req, res) => {
    try {
        ...
        for (const item of records) {
            const existing = await ConstructionLog.findOne({ where: { clientId: item.clientId } });
            if (existing) await existing.update(item);
            else results.push(await ConstructionLog.create(item));
        }
...
router.get('/', authToken, async (req, res) => {
    try {
        const { page = 1, pageSize = 20, teamName, logDateStart, logDateEnd } = req.query;
        const where = {};
        if (teamName) where.teamName = { [Op.like]: `%${teamName}%` };
        if (logDateStart && logDateEnd) {
            where.logDate = { [Op.between]: [logDateStart, logDateEnd] };
        }
        const offset = (page - 1) * pageSize;
        const { count, rows } = await ConstructionLog.findAndCountAll({
            where,
            order: [['logDate', 'DESC']],
            offset,
...
router.get('/:id', authToken, async (req, res) => {
    try {
        const record = await ConstructionLog.findByPk(req.params.id);
